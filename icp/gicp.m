...
p = inputParser;
p.addParameter('MaxIterations', 50, @(x)isnumeric(x));
p.addParameter('MSEThreshold',  1e-6, @(x)isnumeric(x));
p.addParameter('MaxCorrespondenceDist', 1e-4, @(x)isnumeric(x));
p.addParameter('OverlapFraction', 0.8);
p.addParameter('CovarEpsilon',  0.01, @(x)isnumeric(x));
